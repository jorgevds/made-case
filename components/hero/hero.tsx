...
export type DefineMarkersSignature = (stations: BicycleStation[] | undefined) => BikeMapMarker[];

export const defineMarkers: DefineMarkersSignature = (stations?: BicycleStation[]) =>
    stations?.map(station => ({
        position: [station.latitude, station.longitude], popup: {
    ...
    return (
        <article className="h-4/5 w-full bg-amber-100 text-center">
            <section className="flex flex-col m-auto w-4/5">
                {error ? <HeroError /> : isLoading ? <HeroLoading /> :
                    (
                        <>
                            <p className="text-lg font-normal py-8 ">Looking for a <i>velo</i>? Look no further; find one now!</p>
